...
class ArticleCrossrefXmlFilter extends IssueCrossrefXmlFilter
{
    ...
    public function appendCrossmarkNode(DOMDocument $doc, DOMElement $parentNode, array $versionsDois, Publication $publication): void
    {
        ...
        $assertionNode = $doc->createElementNS($deployment->getNamespace(), 'assertion', $publication->getData('datePublished'));
        $assertionNode->setAttribute('name', 'published');
        $assertionNode->setAttribute('label', 'Published');
        $customMetadataNode->appendChild($assertionNode);
